...
Sequential independent instruction are timed.

```c++
auto data = std::vector<int>(10);
switch (num_instructions) // num_instruction is swept
{
  case 1:
  {
  ...
  // ... Ctd with 3, 4 ... using lightweight arithmetic
}
```

## Measurement results

The duration required to perform all independent instructions stays constant up to a number of eight instructions implying that there are eight superscalar pipelines.

```{r}
library(tidyverse)
data <- read.csv("./instruction-level-parallelism.csv") %>%
  as_tibble() %>%
  select(-data.file.path)

ggplot(data) +
  geom_point(aes(x = ninstructions, y = duration_ms)) +
